...
const difyApiKey = 'Ihr-API-Schlüssel'; // Ersetzen Sie 'Ihr-API-Schlüssel' durch Ihren tatsächlichen API-Schlüssel

// Verwenden Sie Dify, um Umgebungsvariablen abzurufen
const value = dify.get('MY_ENV_VARIABLE'); // Ändern Sie 'MY_ENV_VARIABLE' entsprechend Ihrer Umgebungsvariable
console.log('Wert der Umgebungsvariable:', value); // Hier wird der Wert ausgegeben
const query = (obj) =>
    Object.keys(obj)
...
const ask_gpt = async (message) => {
    try {
        ...
        const response = await fetch(`${difyApiEndpoint}/conversation`, {
            method: `POST`,
        ...
        while (true) {
            ...
            if (
                chunk.includes(`<form id="challenge-form" action="${difyApiEndpoint}/conversation?`)
            ) {
